...
export default {
  setup() {
    const { filePath, url, uploadImage } = useStorage('commercials')
    const { addDoc, error } = useCollection('commercials')
    const { user } = getUser()
    ...
    const handleSubmit = async () => {
      if (file.value) {
        ...
        if (!error.value) {
          ...
          router.push({ name: 'Commercial', params: { id: res.id } })
        }
      }
    }
